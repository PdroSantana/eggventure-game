...
namespace Collections.Shaders.CircleTransition
{
	public class CircleTransition : MonoBehaviour
    {
        ...
        public Collider openingActivator;
        public Collider closingActivator;
        ...
        private void Start()
        {
            //escurece fundo desativando fundo provisorio do efeito para melhor transicao
        ...
        private void Update()
        {
            ...
            if (player.GetComponent<EggControl>().IsInTheEnd() == false)
            {
                OpenBlackScreen();
            }
            if (player.GetComponent<EggControl>().IsInTheEnd() == true)
            {
                CloseBlackScreen();
            }
        ...
        {
            DrawBlackScreen();
            StartCoroutine(Transition(0.9f, 0, 1));
        }

        public void CloseBlackScreen()
        {
            DrawBlackScreen();
            StartCoroutine(Transition(0.9f, 1, 0));
        }
